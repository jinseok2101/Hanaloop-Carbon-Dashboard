...
## ⏱️ 작업 소요 시간 및 집중 구현 영역 (Time & Efforts)
- **총 소요 시간**: 약 6.5시간 (기획, 설계, 핵심 코드 구현 및 검증, 문서화 포함)
- **시간이 가장 많이 소요된 영역 (집중 구현 부분)**:
  1. **네트워크 예외 처리 및 트랜잭션 롤백 (약 2.5시간)**: 15%의 확률로 쓰기 에러가 임의 발생하는 불안정한 모의 API 환경에 대응하기 위해, 전역 React Context 단에서 **낙관적 업데이트(Optimistic UI)와 API 실패 시의 롤백(Rollback) 상태 스냅샷 구조**를 설계하고, 다수의 컴포넌트가 실시간 데이터와 동기화되도록 데이터 흐름을 정교화하는 데 가장 많은 시간을 투입했습니다.
  2. **비전문가용 실시간 탄소 연산 및 동적 시각화 (약 2시간)**: 도메인 지식이 없는 경영진도 입력값을 조절할 때마다 예상 수치가 지연 없이 즉각 자동 연산되는 반응형 로직을 확보하고, `원소재 ➔ 생산 ➔ 유통`으로 이어지는 Cradle-to-Gate 라이프사이클 로드맵 및 적층 비중 막대(Stacked share bar) 차트를 순수 CSS 미세 인터랙션과 함께 코드로 동적 렌더링하는 영역에 고도의 집중을 쏟았습니다.
